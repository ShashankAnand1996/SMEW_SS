...
def dis_rock_number_from_mass(mass_distribution_rock, diameter_rock, density_rock):
    number_distribution = mass_distribution_rock / (density_rock*((np.pi*(diameter_rock)**3)/6))
    number_distribution = np.nan_to_num(number_distribution, nan=0.0, posinf=0.0, neginf=0.0)
...
def cumulative_area(xg, yg):
    ...
    for i in range(len(xg)):
        # Get current subarrays
        x_sub = xg[:i+1]
        y_sub = yg[:i+1]

        # Remove consecutive duplicate x values to prevent zero-width intervals
        valid_idx = np.where(np.diff(x_sub, prepend=np.nan) != 0)[0]
        x_valid = x_sub[valid_idx]
        y_valid = y_sub[valid_idx]

        # Only integrate if at least two unique x points
        if len(x_valid) > 1:
            cum_area[i] = simpson(y=y_valid, x=x_valid)
        else:
            cum_area[i] = 0.0  # or np.nan, based on your use case

    return cum_area

#shashank
def cumulative_area_index(xg, yg, aint):
    # Calculate the cumulative area using Simpson's rule
    cumulative_area = np.zeros_like(xg)
    for i in range(len(xg)):
        cumulative_area[i] = simpson(y = yg[:i+1], x = xg[:i+1])
    
    # Retrieve the corresponding value of 'xa' at this index
    return xg[np.argmax(cumulative_area >= aint)], np.argmax(cumulative_area >= aint)

#shashank
def predict_Xi(pore_r, pore_dis, smg, rock_r, rock_dis):
    # This function uses pore_scale distribtuion to provide the largest pore occupied at a given soil moisture
    if smg>=1:
        rw = pore_r[-1]
    else:
        rw = cumulative_area_index(pore_r,pore_dis,smg)[0]
    ...
    if rw >= rock_r[-1]:
        ...
        idx = np.searchsorted(rock_r, rw)
        Xi  = cumulative_area(rock_r,rock_r**2*rock_dis)[idx] / cumulative_area(rock_r,rock_r**2*rock_dis)[-1]
        
    return Xi


def predict_Xi(pore_r, pore_dis, smg, rock_r, rock_dis, alf, lmax):
# This function uses pore_scale distribution to provide the largest pore occupied at a given soil moisture
    c    =  (1-alf)*lmax
    if smg>=1:
        rw = pore_r[-1]
    else:
    ...
    if rw >= rock_r[-1]:
        ...
        idx = np.searchsorted(alf*rock_r+c, rw)
        Xi = cumulative_area(alf*rock_r+c, rock_r**2*(1/alf)*rock_dis)[idx] / cumulative_area(alf*rock_r+c, rock_r**2*(1/alf)*rock_dis)[-1]   
    return Xi

def biogeochem_balance_SS(n, s, L, T, I, v, k_v, RAI, root_d, Zr, r_het, r_aut, D, temp_soil, pH_in, conc_in, f_CEC_in, K_CEC, CEC_tot, Si_in, CaCO3_in, MgCO3_in, M_rock_in, t_app, mineral, rock_f_in, d_in, psd_perc_in, SSA_in, diss_f, pore_d_in, pore_pdf_in, rho_rock_in, mixalf_in, dt, conv_Al, conv_mol, keyword_add):
    ...
    Xi_arr = np.zeros(len(s)) #shashank
...
#------------------------------------------------------------------------------
    ...
    #rock density
    rho_rock = rho_rock_in #3*1e6 # [g/m3] shashank
...
#------------------------------------------------------------------------------            
    ...
    if M_rock_in > 0:
        ...
        psd_rock_num[:,tt_app] = dis_rock_number_from_mass(psd[:,tt_app], d[:,tt_app], rho_rock) #shashank
        ...
        Xi_arr[tt_app] = predict_Xi(pore_d_in, pore_pdf_in, s[0], d[:,tt_app], psd_rock_num[:,tt_app], mixalf_in, d[-1,tt_app]) #shashank
                    
        #mineral weathering
        if t_app == 0:
            for j in range(0, number_min):
                Omega[j,0] = pyEW.Omega_sil(mineral[j], Ca[0], Mg[0], K[0], Na[0], Si[0], H[0], K_sp[j], conv_mol) #[-]
                Wr[j,0] = diss_f*(k_diss_H_t[j,0]*(H[0]/conv_mol)**n_H[j]+k_diss_w_t[j,0]+k_diss_OH_t[j,0]*(H[0]/conv_mol)**n_OH[j])*(1-Omega[j,0]) # [mol-conv/ m2 d]
                EW[j,0] = Wr[j,0]*SA[0]*rock_f[j,0]* Xi_arr[0] # [mol-conv/d]              
...
#------------------------------------------------------------------------------
    ...
    for i in range(1, len(s)):
            ...
            if M_rock_in > 0:
                ...
                #application time
                if i == tt_app:
                    print("HO GYA HIT") #shashank
                    for j in range(0, number_min):
                        Omega[j,i] = pyEW.Omega_sil(mineral[j], Ca[i], Mg[i], K[i], Na[i], Si[i], H[i], K_sp[j], conv_mol) #[-]
                        Wr[j,i]    = diss_f*(k_diss_H_t[j,i]*(H[i]/conv_mol)**n_H[j]+k_diss_w_t[j,i]+k_diss_OH_t[j,i]*(H[i]/conv_mol)**n_OH[j])*(1-Omega[j,i]) # [mol-conv/ m2 d]
                        EW[j,i] = Wr[j,i]*SA[i]*rock_f[j,i]*Xi_arr[i] # [mol-conv/d]
                
                #post application
                elif i > tt_app:
                
                    #diameter class variation
                    d_shrink           = np.sum(rock_f[:,i-1] * Xi_arr[i-1] * Wr[:,i-1] * MM_min[:] / rho_rock) * dt # [m] #shashank
                    ...
                    psd_rock_num[:,i] = dis_rock_number_from_mass(psd[:,i], d[:,i], rho_rock) #shashank
                    Xi_arr[i]         = predict_Xi(pore_d_in, pore_pdf_in, s[i], d[:,i], psd_rock_num[:, i], mixalf_in, d[-1,i]) #shashank

                    #mineral weathering
                    for j in range(0, number_min):
                        if M_min[j,i-1] != 0: 
                            Omega[j,i] = pyEW.Omega_sil(mineral[j], Ca[i], Mg[i], K[i], Na[i], Si[i], H[i], K_sp[j], conv_mol) #[-]
                            Wr[j,i] = diss_f*(k_diss_H_t[j,i]*(H[i]/conv_mol)**n_H[j]+k_diss_w_t[j,i]+k_diss_OH_t[j,i]*(H[i]/conv_mol)**n_OH[j])*(1-Omega[j,i]) # [mol/ m2 d]
                    ...
                    EW[:,i] = Wr[:,i]*SA[i]*rock_f[:,i]*Xi_arr[i] # [mol/d] #shashank
