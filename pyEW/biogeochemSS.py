# -*- coding: utf-8 -*-
"""
Created on April 2024 14:34:44 2025

Author: Shashank Kumar Anand
"""
...
#------------------------------------------------------------------------------
    ...
    #rock density
    rho_rock = rho_rock_in #3*1e6 # [g/m3]
...
#------------------------------------------------------------------------------            
    ...
    if M_rock_in > 0:
        ...
        psd_rock_num[:,tt_app] = dis_rock_number_from_mass(psd[:,tt_app], d[:,tt_app], rho_rock)
        ...
        Xi_arr[tt_app] = predict_Xi(pore_d_in, pore_pdf_in, s[0], d[:,tt_app], psd_rock_num[:,tt_app], mixalf_in, d[-1,tt_app])
    ...
    for i in range(1, len(s)):
            ...
            if M_rock_in > 0:
                ...
                #post application
                    ...
                    d_shrink           = np.sum(rock_f[:,i-1] * Xi_arr[i-1] * Wr[:,i-1] * MM_min[:] / rho_rock) * dt # [m] 
                    ...
                    psd_rock_num[:,i] = dis_rock_number_from_mass(psd[:,i], d[:,i], rho_rock)
                    Xi_arr[i]         = predict_Xi(pore_d_in, pore_pdf_in, s[i], d[:,i], psd_rock_num[:, i], mixalf_in, d[-1,i])
                    ...
                    EW[:,i] = Wr[:,i]*SA[i]*rock_f[:,i]*Xi_arr[i] # [mol/d]
